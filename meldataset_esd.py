...
class MelDataset(torch.utils.data.Dataset):
    ...
    def _get_speech_info(self, speech_file_path):
        split = speech_file_path[:-4].split('/')[-1].split('_')
        speaker_id = split[0]
        sentence_id = split[-1]
    ...
    def _choose_parallel_samples(self, source_speech, target_data_list):
        source_file_path, label = source_speech
        source_id, sentence_id = self._get_speech_info(source_file_path)
        samples = []

        for t in target_data_list:
            t_path = t[0]
            t_speaker_id, t_sentence_id = self._get_speech_info(t_path)
            if t_speaker_id == source_id and t_sentence_id == sentence_id:
                samples.append(t)
    ...
    def __getitem__(self, idx):
        ...
        ref2_data = random.choice(self._choose_parallel_samples(ref_data, self.data_list_per_class[ref_label]))
