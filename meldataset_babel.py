...
class MelDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.data_list = [(path, int(label)) for path, label in _data_list if int(label) == 0]
        self.target_data_list = [(path, int(label)) for path, label in _data_list if int(label) != 0]
    ...
    def __getitem__(self, idx):
        ...
        ref2_data = random.choice(self.target_data_list)
